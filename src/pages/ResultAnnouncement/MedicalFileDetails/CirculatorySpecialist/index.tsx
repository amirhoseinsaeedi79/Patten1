...
const CirculatorySpecialist = () => {
  ...
  const {
    ...
    mutationKey: ["CirculatorySpecialist Data", MFD_And_MFS_JobId_Get],
    mutationFn: async () => {
      const response = await smartRequest(MFD_And_MFS_JobId_Get)
      // console.log("CirculatorySpecialist Data Response:", response.data.data);
  ...
  const pageKeyMapping: { [key: number]: string[] } = {
    1: ["1690178062123"], // keys for CirculatorySpecialist
  }

  // Extract the relevant keys for the CirculatorySpecialist page
  const currentPageKeys = pageKeyMapping[1] // Assuming CirculatorySpecialist page corresponds to index 1
  ...
  return (
    <div className={styles.container}>
      <Toastify id={41} getError={circError} />
      <h1>{text.s1}</h1>
      {circSuccess && (
        <div className={styles.bigContent}>
          <Textarea
            label={text.s2}
            value={currentPageData["1690178062123"]!}
...
export default CirculatorySpecialist
